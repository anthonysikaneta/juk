(function () {
	...
	function MusicService ($http) {
		...
		var getSongs = function () {
			return $http.get('http://ws.audioscrobbler.com/2.0/?method=geo.gettopartists&country=canada&api_key=4b88034a7ebabe81bc700db8b7f491a5&format=json').then(function(response) {
	...
	function JukExtra ($http, $q) {
		...
		var getLastFM = function () {
			return $http.get('http://ws.audioscrobbler.com/2.0/?method=tag.gettopartists&format=json&tag=disco&&api_key=8c271cb9401f88c39a453a7a07b48d31&cb=http://localhost:9393/#/tab/songs').then(function(response) {
				return response.data.topartists.artist;
			}, function (status) {
				console.log(status);
			});
